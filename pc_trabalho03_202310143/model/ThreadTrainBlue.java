...
public class ThreadTrainBlue extends Thread {
    ...
    private boolean ativa = true; // indica se a thread do trem azul esta ativa (inicializada como ativa)
    private int exclusao; // determina a variavel do tipo de exclusao mutua
    private int processo = 1; //variavel do processo atual (para o trem vermlho = 0, para o trem azul = 1)
    private boolean criticZone1 = false; // indica se o trem azul esta na zona critica 1
    private boolean criticZone2 = false; // indica se o trem azul esta na zona critica 2

    /****************************************************************
     * Metodo: run
     * Funcao: define o que a thread vai controlar que nesse caso e o movimento
     * continuo do trem azul
     * Parametros:void
     * Retorno: void
     ****************************************************************/
    @Override
    public void run() {
     ...
     // contrutor que define a posicao inicial do trem azul, a sua imagem, o seu slider e o tipo da exclusao mutua
    public ThreadTrainBlue(ImageView imageTrain, int posicao, Slider slider, int exclusao) {
        this.imageTrain = imageTrain;
        this.posicao = posicao;
        this.slider = slider;
        this.exclusao = exclusao;

        //determina a posicao inicial assumida pelo trem azul
        switch (posicao) {
            case 2: {
    ...
    public int getPosicao() {
     ...
     /****************************************************************
     * Metodo: entraRC1
     * Funcao: Verifica se o trem azul pode acessar a regiao critica 1
     * Parametros: int - o tipo de exclusao escolhida no construtor da thread
     * Retorno: boolean - retorna true se o trem azul puder acessar e false se nao puder acessar
     ****************************************************************/
    private boolean entraRC1(int exclusao) {
        switch (exclusao) {
            ...
            case 1: {
             ...
             //Estrita Alternancia
            case 2: {
                if (ControllerTrains.turn1 == 0) {
                    // Retorna falso se for o turno do outro trem
                    return false;
                } else {
                     // Marca que o trem azul esta na zona critica 1
                    criticZone1 = true;
                    // Retorna true que o trem azul pode acessar a zona critica 1
                    return true;
                }
            }
    ...
    private void saiRC1(int exclusao) {
        switch (exclusao) {
            //Variavel de travamento
            case 1: {
                // Libera o travamento para permitir que o trem vermelho acesse;
    ...
    /****************************************************************
     * Metodo: entraRC2
     * Funcao: Verifica se o trem azul pode acessar a regiao critica 2
     * Parametros: int - o tipo de exclusao escolhida no construtor da thread
     * Retorno: boolean - retorna true se o trem azul puder acessar e false se nao puder acessar
     ****************************************************************/
    private boolean entraRC2(int exclusao) {
        switch (exclusao) {
            ...
            case 1: {
             ...
             //Estrita Alternancia
            case 2:{
                if(ControllerTrains.turn2 == 0 ){
                    // Retorna falso se for o turno do outro trem
                    return false;
                } else {
                    // Marca que o trem azul esta na zona critica 2
                    criticZone2 = true;

                    // Retorna true que o trem azul pode acessar a zona critica 2
                    return true;
                }
              }
              case 3:{
                ...
                if(ControllerTrains.turnPeterson2 == processo && ControllerTrains.interesse2[other] == true){
                    ...
                    // Marca que o trem azul esta na regi�o cr�tica 2
                    criticZone2 = true;
                    // Retorna true que o trem azul pode acessar a zona critica 2
                    return true;
                }
    ...
    /****************************************************************
     * Metodo: moveTrain
     * Funcao: responsavel por mover o trem azul de acordo com sua posicao
     * escolhida
     * Parametros:void
     * Retorno: void
     ****************************************************************/
    public void moveTrain() {
        switch (posicao) {
            case 2: {
                // Trem Blue - descendo
                if (this.getImageTrem().getLayoutY() < 20 && this.getImageTrem().getLayoutY() >= -29) {
                ...
                // REGIAO CRITICA 1
                    ...
                    //se o trem azul nao pode entrar na regiao critica e a zona critica 1 nao esta ocupada
                ...
                else if (this.getImageTrem().getLayoutY() < 215 && this.getImageTrem().getLayoutY() >= 190) {
                ...
                else if (this.getImageTrem().getLayoutY() < 275 && this.getImageTrem().getLayoutY() >= 215) {
                    this.getImageTrem().setRotate(0);
                    this.getImageTrem().setLayoutY(this.getImageTrem().getLayoutY() + this.getSpeedY());
                } 
                else if (this.getImageTrem().getLayoutY() < 300 && this.getImageTrem().getLayoutY() >= 275) {
                    this.getImageTrem().setRotate(45);
                    this.getImageTrem().setLayoutY(this.getImageTrem().getLayoutY() + this.getSpeedY());
                    this.getImageTrem().setLayoutX(this.getImageTrem().getLayoutX() - this.getSpeedX());
                } 
                ...
                /************************************************************************************************* */
                // REGIAO CRITICA 2
                else if (this.getImageTrem().getLayoutY() < 333 && this.getImageTrem().getLayoutY() >= 300) {
                    //se o trem azul nao pode entrar na regiao critica e a zona critica 2 nao esta ocupada
                ...
                else if (this.getImageTrem().getLayoutY() < 469 && this.getImageTrem().getLayoutY() >= 448) {
                ...
                else if (this.getImageTrem().getLayoutY() < 550 && this.getImageTrem().getLayoutY() >= 469) {
                    this.getImageTrem().setRotate(0);
                    this.getImageTrem().setLayoutY(this.getImageTrem().getLayoutY() + this.getSpeedY());
                } // fim do nono if
                else {
                    this.getImageTrem().setLayoutX(506);
                    this.getImageTrem().setLayoutY(-29);
                    this.getImageTrem().setRotate(0);
                } // fim do else
                break;
            }
            case 4: {
                // Trem Blue - Subindo
                if (this.getImageTrem().getLayoutY() <= 523 && this.getImageTrem().getLayoutY() > 470) {
                    this.getImageTrem().setLayoutY(this.getImageTrem().getLayoutY() - this.getSpeedY());
                } 
                ...
                else if (this.getImageTrem().getLayoutY() <= 45 && this.getImageTrem().getLayoutY() > 25) {
                    //libera a regiao critica 1
                    saiRC1(exclusao);
                    this.getImageTrem().setRotate(225);
                    this.getImageTrem().setLayoutY(this.getImageTrem().getLayoutY() - this.getSpeedY());
                ...
                else if (this.getImageTrem().getLayoutY() <= 25 && this.getImageTrem().getLayoutY() > -47) {
                    this.getImageTrem().setRotate(180);
                    this.getImageTrem().setLayoutY(this.getImageTrem().getLayoutY() - this.getSpeedY());
                } 
                else {
                    this.getImageTrem().setLayoutX(506);
                    this.getImageTrem().setLayoutY(523);
                    this.getImageTrem().setRotate(180);
                } 
                break;
            }
